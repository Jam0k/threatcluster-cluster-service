"""
Scheduler for entity notification emails
Runs periodically to check for entity updates in custom feeds and send notifications
"""
...
async def run_notification_check():
    """Run the entity notification check"""
    logger.info("Starting entity notification check...")
    
    try:
        service = EntityNotificationService()
        await service.check_all_users_for_entity_updates()
        logger.info("Entity notification check completed successfully")
...
async def run_scheduler(interval_minutes: int = 30):
    ...
    logger.info(f"Starting entity notification scheduler (interval: {interval_minutes} minutes)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ThreatCluster Entity Notification Scheduler"
    )
    parser.add_argument(
        "--once",
        action="store_true",
